...
# Two cold pool configuration set up by Greg Dritschel
...
try:
    ...
    # domain extent - only simulate the mixed layer
    extent = np.array([4*z_b, z_b, z_b])
    # redefine cold pool bubble radius
    r_plume = z_b/4 # just needs to be smaller than xp0, so that bubbles are not overlapping
    ...
    # define plume offsets in horizontal and vertical
    xp0 = x_b/4 # half of the domain width - otherwise two thin a spread?!
    zp0 = -z_b/4 # hemispheric cold pools
    bratio = 1.5 # bubbles ratio of buoyancy
    hratio = 1.0
    print("r_plume - xp0   =" + str(r_plume - xp0))
    print("Box layout:")
    print("x_max           =" + str(extent[0]))
    ...
    print("offset in x     =" + str(xp0))
    print("top of plume    =" + str(2.0 * r_plume + zp0))
    print("bottom of plume =" + str(0.0 + zp0))
    ...
    fac = 10 # scale how negatively buoyant the cold pools are!
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            for k in range(nz+1):
                ...
                # Second bubble - denser than the first
                elif r21 <= radsq*r_smooth_frac*r_smooth_frac:
                    buoy[k, j, i] = - fac * b_pl * bratio * (
                        1.0
                        + e_values[0] * rx1 * ry1
                        + e_values[1] * rx1 * rz1
                        + e_values[2] * ry1 * rz1
                    ...
                    # relative position on smoothed edge of bubble
                    r_edge = (np.sqrt(r21)-r_plume*r_smooth_frac)/(r_plume*(1.0-r_smooth_frac))
                    # use fifth order smoothstep function on edge
                    buoy[k, j, i] = - fac * b_pl * bratio * (
                        1.0
                        + e_values[0] * rx1 * ry1
    ...
    ncf.open('two_cps_' + str(nx) + 'x' + str(ny) + 'x' + str(nz) + '.nc')
