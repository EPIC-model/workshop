...
```
You can view your home file system quota and use through SAFE.
Use the *Login account* menu to select the account you want to see the information for.
The account summary page will contain information on your home file system use and any quotas (user or project) that apply to that account.
Note that SAFE home file system use data is updated daily so the information may not quite match the state of the system if a large change has happened recently.
Quotas will be completely up to date as they are controlled by SAFE.

### Home file system and Home directory

A potential source of confusion is the distinction between
the *home file system* which is the storage system on Cirrus used for critical data
and your *home directory* which is a Linux concept of the directory that you are placed into when you first login,
that is stored in the `$HOME` environment variable and that can be accessed with the `cd ~` command.

## Work

The work file systems, which are available on the login, data analysis and compute nodes,
are designed for high performance parallel access and are the primary location that jobs running on the compute nodes will read data from and write data to.
They are based on the Lustre parallel file system technology.
The work file systems are not backed up in any way.
There is a total of 24 PB usable space available on the work file systems.
...
For example, if your username is `auser` and you are in the project `d185` then your main work directory will be at:
...
Both the home and work file systems have special directories that allow you to share data with other users.
There are directories that allow you to share data only with other users in the same project and directories that allow you to share data with users in other projects.

To share data with users in the same project you use the `/work/d185/d185/shared` directory
and make sure the permissions on the directory are correctly set to allow sharing in the project:

```bash
auser@cirrus-login1:~> mkdir /work/d185/d185/shared/interesting-data
auser@cirrus-login1:~> cp -r modelling-output /work/d185/d185/shared/interesting-data/
auser@cirrus-login1:~> chmod -R g+rX,o-rwx /work/d185/d185/shared/interesting-data
auser@cirrus-login1:~> ls -l /work/d185/d185/shared
total 150372
...
and make sure the permissions on the directory are correctly set to allow sharing with all other users:

```bash
auser@cirrus-login1:~> mkdir /work/d185/shared/more-interesting-data
auser@cirrus-login1:~> cp -r more-modelling-output /work/d185/shared/more-interesting-data/
auser@cirrus-login1:~> chmod -R go+rX /work/d185/shared/more-interesting-data
auser@cirrus-login1:~> ls -l /work/d185/shared
total 150372
...
The Cirrus login and compute nodes have access to a shared, high-performance, solid state storage system (also known as RPOOL).
This storage system is network mounted and shared across the login nodes and GPU compute nodes in a similar way to the normal,
...
```bash
auser@cirrus-login1:~> cd /scratch/space1/d185
```

If this is your first time in this directory, you will need to create a directory to work from.
You can do this by running:

```bash
auser@cirrus-login1:~> mkdir $USER
auser@cirrus-login1:~> cd $USER
```

You can transfer data locally to this directory by running:

```bash
auser@cirrus-login1:~> cp -r /path/to/data-dir /scratch/space1/d185/$USER/
```

For remote transfers to this directory, you will need to use `rscync` or `scp` --
you can use the following command as an example (making sure to replace `<username>` with your Cirrus username):
...
The service officially supports the *bash* shell for interactive access, shell scripting and job submission scripts.
The scheduling software is Slurm.
...
In addition to this, the EPCC Cirrus CSE service have installed a wide range of
modelling and simulation software, additional scientific and numeric libraries, data analysis tools and other useful software.
Some examples of the software installed are:
...
More information on the software available on Cirrus can be found in [the Cirrus Documentation](https://docs.cirrus.ac.uk).
